...
if(isset($_POST['btnok'])){
    ...
    if ($login==false){
        echo("<script>alert('$msg_erro'); location.href='nossas_lojas.php';</script>");
    }
...
<html>
    ...
    <body>
       ...
       <header>
            <form name="frmlogin" method="post" action="nossas_lojas.php">
                <div id="cabecalho">
                    <div id="logo">
                        <a href="index.php">
        ...
        <section id="content_nossas_lojas">
            <div class="titulo_pagina">
                Nossas Lojas
            </div>
            <?php
            $sql1 = "SELECT * FROM tbl_lojas where status='1'";
            $select = mysqli_query($conexao, $sql1);
            while($rsLoja = mysqli_fetch_array($select)){
            ?>
                <div class="lojas">
                    <div class="cidade_loja">
                        <?php echo($rsLoja['cidade']); ?>
                    </div>
                    <div class="endereco_telefone_loja">
                        <?php echo($rsLoja['logradouro'].", nº ".$rsLoja['numero']." - ".$rsLoja['bairro']); ?>
                        <p><?php echo($rsLoja['telefone1']); ?></p>
